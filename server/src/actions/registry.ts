...
export const ACTION_DEFINITIONS: readonly ActionDefinition[] = [
  ...
  // --- Outlook Mail (unified Microsoft provider) -----------------------
  {
    actionId: "microsoft.mail.createDraft",
    category: "email",
    displayName: "Create Outlook draft",
    description: "Create a new, reply, reply-all, or forward draft in the connected Microsoft mailbox. Never sends.",
    providerTypes: ["microsoft"],
    requiredCapabilities: ["outlook_mail.write"],
    ...
    enabled: true,
    inputSchema: [
      { name: "operation", type: "string", required: true, description: "Resolved draft operation." },
      { name: "sourceMessageId", type: "string", required: false, description: "Immutable source message id for reply/forward drafts." },
      { name: "to", type: "string[]", required: false, description: "Explicit or authoritative recipient addresses." },
      { name: "subject", type: "string", required: false, description: "Subject for a new draft." },
      { name: "body", type: "string", required: true, description: "Draft body." },
    ],
    examples: ["draft an Outlook email", "write an Outlook reply"],
    userFacingDescription: "I couldn’t create that Outlook draft just now.",
  },
  {
    actionId: "microsoft.mail.updateDraft",
    category: "email",
    displayName: "Edit Outlook draft",
    description: "Update one authoritatively resolved unsent Outlook draft.",
    providerTypes: ["microsoft"],
    requiredCapabilities: ["outlook_mail.write"],
    requiredScopes: ["Mail.ReadWrite"],
    riskLevel: "draft",
    ...
    enabled: true,
    inputSchema: [
      { name: "draftId", type: "string", required: true, description: "Immutable draft id." },
      { name: "body", type: "string", required: true, description: "Complete replacement body." },
    ],
    examples: ["make that Outlook draft shorter"],
    userFacingDescription: "I couldn’t update that Outlook draft just now.",
  },
  {
    actionId: "microsoft.mail.deleteDraft",
    category: "email",
    displayName: "Delete Outlook draft",
    description: "Delete one authoritatively resolved unsent Outlook draft after confirmation.",
    providerTypes: ["microsoft"],
    requiredCapabilities: ["outlook_mail.write"],
    requiredScopes: ["Mail.ReadWrite"],
    riskLevel: "write",
    ...
    implemented: true,
    enabled: true,
    inputSchema: [{ name: "draftId", type: "string", required: true, description: "Immutable draft id." }],
    examples: ["delete that Outlook draft"],
    userFacingDescription: "I couldn’t delete that Outlook draft just now.",
  },
  {
    actionId: "microsoft.mail.setReadState",
    category: "email",
    displayName: "Change Outlook read state",
    description: "Mark one authoritatively resolved Outlook message read or unread, then verify it.",
    providerTypes: ["microsoft"],
    requiredCapabilities: ["outlook_mail.write"],
    requiredScopes: ["Mail.ReadWrite"],
    riskLevel: "modify",
    ...
    enabled: true,
    inputSchema: [
      { name: "operation", type: "string", required: true, description: "mark_read or mark_unread." },
      { name: "messageId", type: "string", required: true, description: "Immutable message id." },
    ],
    examples: ["mark that Outlook message unread"],
    userFacingDescription: "I couldn’t verify that Outlook read-state change.",
  },
  {
    actionId: "microsoft.mail.send",
    category: "email",
    displayName: "Send Outlook email",
    description: "Send a new Outlook message, reply, reply-all, forward, or existing draft after exact confirmation.",
    providerTypes: ["microsoft"],
    requiredCapabilities: ["outlook_mail.write", "outlook_mail.send"],
    ...
    enabled: true,
    inputSchema: [
      { name: "operation", type: "string", required: true, description: "Resolved send operation." },
      { name: "sourceMessageId", type: "string", required: false, description: "Immutable reply/forward source id." },
      { name: "draftId", type: "string", required: false, description: "Immutable existing draft id." },
      { name: "to", type: "string[]", required: false, description: "Explicit or authoritative recipients." },
      { name: "subject", type: "string", required: false, description: "Subject for a new message." },
      { name: "body", type: "string", required: false, description: "Exact proposed body." },
    ],
    examples: ["send an Outlook email", "reply from Outlook", "forward that Outlook message"],
    userFacingDescription: "I couldn’t send that Outlook message just now.",
  },
  {
    actionId: "microsoft.calendar.mutate",
    category: "calendar",
    displayName: "Change Outlook calendar",
    description: "Create, update, reschedule, add a Teams meeting to, or cancel one authoritatively resolved Outlook event after confirmation.",
    providerTypes: ["microsoft"],
    requiredCapabilities: ["outlook_calendar.write"],
    ...
    enabled: true,
    inputSchema: [
      { name: "operation", type: "string", required: true, description: "create, update, or delete." },
      { name: "eventId", type: "string", required: false, description: "Immutable event id for update/delete." },
      { name: "transactionId", type: "string", required: false, description: "Durable Graph create idempotency key." },
      { name: "title", type: "string", required: false, description: "Resolved event title." },
      { name: "start", type: "datetime", required: false, description: "Resolved start instant." },
      { name: "end", type: "datetime", required: false, description: "Resolved end instant." },
      { name: "timeZone", type: "string", required: false, description: "User IANA timezone used to interpret wall time." },
      { name: "attendees", type: "string[]", required: false, description: "Exact attendee email addresses." },
      { name: "teamsMeeting", type: "boolean", required: false, description: "Request a Teams link through the event API." },
    ],
    examples: ["schedule an Outlook event", "move that Microsoft meeting", "make it a Teams call", "cancel that Outlook event"],
    userFacingDescription: "I couldn’t verify that Outlook Calendar change.",
  },
  ...
  },
  {
    ...
    providerTypes: ["notion"], requiredCapabilities: ["content.write"], requiredScopes: ["content:write"],
    riskLevel: "write", confirmationRequired: true, implemented: true, enabled: true,
    inputSchema: [{name:"operation",type:"string",required:true,description:"Allowlisted mutation"},{name:"targetId",type:"string",required:false,description:"Resolved target"},{name:"body",type:"object",required:true,description:"Validated Notion payload"},{name:"targetTitle",type:"string",required:true,description:"Safe target title"}],
    examples:["archive that Notion page","comment on that Notion page","change that data source schema"],
    userFacingDescription:"I couldn’t verify that Notion change.",
  },
  {
  ...
  // --- Messaging ---------------------------------------------------------
  {
    ...
    providerTypes: ["slack"],
    requiredCapabilities: ["slack.messages.write"],
    ...
    userFacingDescription: "I couldn’t post that Slack message safely.",
  },
  {
    actionId: "slack.mutate",
    category: "messaging",
    displayName: "Change Slack",
    description: "Perform one fully resolved and confirmed Slack mutation.",
    providerTypes: ["slack"],
    requiredCapabilities: ["slack.read"],
    requiredScopes: [],
    riskLevel: "write",
    confirmationRequired: true,
    implemented: true,
    enabled: true,
    inputSchema: [
      { name: "method", type: "string", required: true, description: "Allowlisted Slack method." },
      { name: "params", type: "object", required: true, description: "Resolved provider parameters." },
      { name: "successText", type: "string", required: true, description: "Safe receipt copy." },
    ],
    examples: ["react to that Slack message", "archive that Slack channel"],
    userFacingDescription: "I couldn’t make that Slack change safely.",
  },
  // --- Shopping ----------------------------------------------------------
  {
