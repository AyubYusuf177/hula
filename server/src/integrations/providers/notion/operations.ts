...
export const notionOps={
  ...
  user:(u:string,userId:string,d?:NotionOpsDeps)=>notionRequest<NotionObject>(u,{method:"GET",path:`/users/${id(userId)}`,safeRetry:true},d),
  ...
  createDatabase:async(u:string,body:Record<string,unknown>,d?:NotionOpsDeps)=>receipt(await notionRequest(u,{method:"POST",path:"/databases",body,idempotencyKey:randomUUID()},d),"database"),
  updateDatabase:async(u:string,databaseId:string,body:Record<string,unknown>,d?:NotionOpsDeps)=>targetReceipt(await notionRequest(u,{method:"PATCH",path:`/databases/${id(databaseId)}`,body},d),databaseId,"database"),
  ...
  appendBlocks:async(u:string,blockId:string,children:unknown[],d?:NotionOpsDeps,position?:Record<string,unknown>)=>notionRequest<NotionObject>(u,{method:"PATCH",path:`/blocks/${id(blockId)}/children`,body:{children,...(position?{position}:{})},idempotencyKey:randomUUID()},d),
  ...
  views:(u:string,parent:{database_id?:string;data_source_id?:string},d?:NotionOpsDeps)=>notionPaginate(u,{method:"GET",path:"/views",query:parent,safeRetry:true},{maxResults:100,deps:d}),
  view:(u:string,viewId:string,d?:NotionOpsDeps)=>notionRequest<NotionObject>(u,{method:"GET",path:`/views/${id(viewId)}`,safeRetry:true},d),
  createView:async(u:string,body:Record<string,unknown>,d?:NotionOpsDeps)=>receipt(await notionRequest(u,{method:"POST",path:"/views",body,idempotencyKey:randomUUID()},d),"view"),
  updateView:async(u:string,viewId:string,body:Record<string,unknown>,d?:NotionOpsDeps)=>targetReceipt(await notionRequest(u,{method:"PATCH",path:`/views/${id(viewId)}`,body},d),viewId,"view"),
  deleteView:async(u:string,viewId:string,d?:NotionOpsDeps)=>targetReceipt(await notionRequest(u,{method:"DELETE",path:`/views/${id(viewId)}`},d),viewId,"view"),
  createViewQuery:async(u:string,viewId:string,pageSize=100,d?:NotionOpsDeps)=>receipt(await notionRequest(u,{method:"POST",path:`/views/${id(viewId)}/queries`,body:{page_size:Math.min(Math.max(pageSize,1),100)},safeRetry:true},d),"view_query"),
  viewQueryResults:(u:string,viewId:string,queryId:string,d?:NotionOpsDeps)=>notionPaginate(u,{method:"GET",path:`/views/${id(viewId)}/queries/${id(queryId)}`,safeRetry:true},{maxResults:1000,deps:d}),
  deleteViewQuery:async(u:string,viewId:string,queryId:string,d?:NotionOpsDeps)=>targetReceipt(await notionRequest(u,{method:"DELETE",path:`/views/${id(viewId)}/queries/${id(queryId)}`},d),queryId,"view_query"),
  fileUploads:(u:string,d?:NotionOpsDeps)=>notionPaginate(u,{method:"GET",path:"/file_uploads",safeRetry:true},{maxResults:100,deps:d}),
  fileUpload:(u:string,fileId:string,d?:NotionOpsDeps)=>notionRequest<NotionObject>(u,{method:"GET",path:`/file_uploads/${id(fileId)}`,safeRetry:true},d),
